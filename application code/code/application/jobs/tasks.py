...
def schedule_daily_task(sender, **kwargs):
    sender.add_periodic_task(crontab(hour=8, minute=33), daily_email_task.s(), name="daily_email_task")

@celery.on_after_finalize.connect
def schedule_monthly_task(sender, **kwargs):
    sender.add_periodic_task(crontab(day_of_month='11', hour=8, minute=33), monthly_email_task.s(), name="monthly_email_task")

@celery.task
def daily_email_task():
    users = User.query.all()
    for user in users:
        if datetime.now() - user.last_login >= timedelta(hours=0):
...
def monthly_email_task():
    ...
    for sponsor in all_sponsors:
        campaigns = Camp.query.filter_by(sponsor_id=sponsor.id).all()
        campaign_info = []
        total_ads_count = 0
        total_expense = 0.0
        ...
        for campaign in campaigns:
            ad=campaign.ads
            ad_requests = AdsRequest.query.filter_by(ad_id=ad[0].id).all()
            ads_count = len(ad_requests)
            expense = sum(ad.payment for ad in ad_requests)
        ...
        dispatch_email(
            receiver=sponsor.user.email,
            mail_subject="Monthly Activity Report",
            mail_body=email_content
        )

    return "Monthly emails have been sent!"

@celery.task
def create_csv_report(sponsor_id):
    sponsor = Sponsor.query.get(sponsor_id)
    campaigns = Camp.query.filter_by(sponsor_id=sponsor_id).all()

    file_name = f'campaigns_{sponsor_id}.csv'
    with open(file_name, mode='w') as file:
        writer = csv.writer(file)
        writer.writerow(['Title', 'Description', 'Start Date', 'End Date', 'Budget', 'Visibility', 'Goals'])
        for campaign in campaigns:
            writer.writerow([
                campaign.title,
    ...
    with open('templates/exportmail.html') as template_file:
        template = Template(template_file.read())
        email_body = template.render(name=sponsor.user.username)
    ...
    return "CSV report generated and email sent to the sponsor"  
"""
...
def schedule_daily_task(sender, **kwargs):
    ...
    sender.add_periodic_task(crontab(hour=00, minute=47), daily_email_task.s(), name="daily_email_task")
